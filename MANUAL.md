...
- **Niveles acuáticos**: al entrar al agua aparece el estado de oxígeno. Recupera aire en
  tierra seca o junto a las columnas de burbujas; a cero empiezas a sufrir daño por ahogo.
  Las botas reforzadas reducen el peligro de las corrientes, pero no sustituyen el aire.
...
- **Salidas rituales**: algunas salidas no son puertas — son el objeto exacto que dice la wiki
  ...
  documentadas de cada nivel catalogado están conectadas y se pueden atravesar.
...
Sin escribir nada se usa la **semilla diaria**, compartida por todos y renovada a las 00:00
de `Europe/Madrid`. La semilla cambia la forma, distribución y posición de salidas de cada
sala, y fija el destino de las rutas variables durante ese día. En MMO, las salas activas se reorganizan al comenzar el nuevo día.
También puedes escribir una semilla manual (ej. `moqueta-777`): esa no caduca y genera
exactamente los mismos mapas, útil para que tu chat reproduzca una partida concreta.
...
Sin esos pasos el juego y el mapa no ven los cambios. Luego F5 en el navegador.
...
- aparece la fila **🐞 Debug** con el desplegable del catálogo completo y el botón **Teleport**
...
Este archivo representa el grafo de conexiones, no el dibujo de paredes de cada sala. Una
sala puede no tener entradas o salidas documentadas; si la geometría actual no ofrece salida,
el camino por el que llegaste permanece abierto para poder retroceder.

Se regenera con: `node pipeline/make-catalog-map.js`. El mapa reducido del piloto se
regenera con `node pipeline/make-map.js`.

## 9. Actualizar la copia local de la wiki

La copia activa de la wiki (1.134 páginas) está en `data/raw/`. Si quieres refrescarla con
