...
const DailySeed = esNode ? require('../engine/daily-seed') : window.DailySeed;
const RouteSeed = esNode ? require('../engine/route-seed') : window.RouteSeed;
...
function claveInterna(nivelId, inst, grupo, dia = semillaBase === 'mmo' ? DailySeed.dayKey() : 'local') {
...
class Sala {
  constructor(nivelId, inst, grupo = SALA_PUBLICA, semillaBase = 'mmo') {
    ...
    this.dia = semillaBase === 'mmo' ? DailySeed.dayKey() : null;
    ...
    const baseConCiclo = this.dia ? `${semillaBase}::${this.dia}` : semillaBase;
    this.semilla = this.privada
      ? `${baseConCiclo}::privada::${this.grupo}::${nivelId}::${inst}`
      : `${baseConCiclo}::${nivelId}::${inst}`;
  ...
  resolverDestino(jug, def) {
    ...
    else if (def.destino.startsWith('*opciones:'))
      candidatos = def.destino.slice('*opciones:'.length).split(',').filter((id) => DATA.levels[id]);
    if (!candidatos?.length) return null;
    const destino = RouteSeed.pick(DailySeed.seed(), this.nivelId, def, candidatos);
    return { ...def, destino, _destinoResuelto: destino };
  }
  ...
  enviarEstado(jug) {
    this.enviar(jug.ws, {
      t: 'estado', salud: jug.salud, sed: jug.sed, cordura: jug.cordura, oxigeno: jug.oxigeno,
    });
  }
  ...
  tick(ahora) {
    ...
    if ((this.def.reglas || []).includes('respiracion_acuatica')) {
      for (const jug of this.jugadores.values()) {
        if (jug.muerto || jug.espectador || ahora < (jug._oxigenoEn || 0)) continue;
...
function crearSala(nivelId, inst, grupo) {
  ...
  salas.set(claveInterna(nivelId, inst, grupo, sala.dia || 'local'), sala);
...
function moverEspectador(esp, salaVieja, nueva, objetivo) {
  ...
  nueva.enviar(esp.ws, {
    ...
    sinTarjeta: true,
    salud: esp.salud, sed: esp.sed, cordura: esp.cordura, oxigeno: esp.oxigeno,
    inv: esp.inv, manos: esp.manos, equipo: esp.equipo,
